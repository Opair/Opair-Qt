...
static const int64_t MAX_MONEY = 152000000 * COIN; // Max Coins
static const int64_t MAX_MINT_PROOF_OF_STAKE = 0.03 * COIN; // 3% Interest
static const int64_t COIN_YEAR_REWARD = 3 * CENT; // 3% Interest
...
public:
    ...
    {
        // Opair: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // Opair: get transaction coin age
...
public:
    ...
    // Opair: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // Opair: two types of block: proof-of-work or proof-of-stake
    ...
    // Opair: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // Opair: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // Opair: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // Opair: block index flags
    enum
    {
